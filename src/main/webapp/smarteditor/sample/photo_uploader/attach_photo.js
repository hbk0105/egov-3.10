    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/commomnMultiImageUpload.do'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		console.log(JSON.parse(sResString));

		var result = JSON.parse(sResString);
		...
		if(result.result != 'SUCCESS'){
			...
			for(var i = 0; i < list.length; i++){
				var sResString = "bNewLine="+list[i].bNewLine;
				sResString += "&sFileName="+list[i].sFileName;
				sResString += "&sFileURL="+list[i].sFileURL;
    	...
    	var	aTemp = [],
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
				...
				/*	if( aTemp[i].indexOf('sFileURL') != -1){
						aSubTemp = aTemp[i].split("=");
						htTemp[sFileURL] = sFileURL;
					}else if( aTemp[i].indexOf('fileSeCode') == -1){
*/
						aSubTemp = aTemp[i].split("=");
